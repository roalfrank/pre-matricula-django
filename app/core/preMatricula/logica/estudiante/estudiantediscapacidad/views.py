...
from .form import DiscapacidadEstudianteForm
from core.preMatricula.mixis import ValidatePermissionRequiredCrudSimpleMixin


class DiscapacidadEstudianteView(LoginRequiredMixin, ValidatePermissionRequiredCrudSimpleMixin, TemplateView):
    template_name = "estudiante/estudiante-discapacidad/list.html"
    permiso_vista = 'view_discapacidad'
    permiso_crud = {
        'add': 'add_discapacidad',
        'change': 'change_discapacidad',
        'delete': 'delete_discapacidad',
    }
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'add':
                form = DiscapacidadEstudianteForm(request.POST)
                if form.is_valid():
                    form.save()
                    data['enviado'] = True
            ...
            # action chequearEstado , esto es para comprobar rapido si ya hay otra provincia con el Mismo Nombre
                ...
                try:
                    ...
                    estado = Discapacidad.objects.filter(
            ...
            # Action  cargarDatos - para cargar la datatable de mi vista.
                ...
                data = [i.toJson() for i in Discapacidad.objects.all()]
                respuesta = JsonResponse(data, safe=False)
                return respuesta
            # action edit - editando una entidad.
            elif action == 'edit':
                registro = Discapacidad.objects.get(
                    pk=int(request.POST['id_edit']))
                form = DiscapacidadEstudianteForm(
            ...
            # action delete, eliminar una registro
                ...
                try:
                    ...
                    delet_registro = Discapacidad.objects.filter(
                        pk__in=id_deletes)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = "Lista de Discapacidad Estudiante"
        context['icono_titulo'] = "fas fa-tachometer-alt"
        context['form'] = DiscapacidadEstudianteForm()
        return context
