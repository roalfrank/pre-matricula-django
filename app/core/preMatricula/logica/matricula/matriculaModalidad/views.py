...
from .form import ModalidadMatriculaForm
from core.preMatricula.mixis import ValidatePermissionRequiredCrudSimpleMixin


class ModalidadMatriculaView(LoginRequiredMixin, ValidatePermissionRequiredCrudSimpleMixin, TemplateView):
    template_name = "matricula/matricula-modalidad/list.html"
    permiso_vista = 'view_modalidad'
    permiso_crud = {
        'add': 'add_modalidad',
        'change': 'change_modalidad',
        'delete': 'delete_modalidad',
    }
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'add':
                form = ModalidadMatriculaForm(request.POST)
                if form.is_valid():
                    form.save()
                    data['enviado'] = True
            ...
            # action chequear , esto es para comprobar rapido si ya hay otra  con el Mismo Nombre
                ...
                try:
                    ...
                    estado = Modalidad.objects.filter(
            ...
            # Action  cargarDatos - para cargar la datatable de mi vista.
                ...
                data = [i.toJson() for i in Modalidad.objects.all()]
                respuesta = JsonResponse(data, safe=False)
                return respuesta
            # action edit - editando una entidad.
            elif action == 'edit':
                registro = Modalidad.objects.get(
                    pk=int(request.POST['id_edit']))
                form = ModalidadMatriculaForm(
            ...
            # action delete, eliminar una registro
                ...
                try:
                    ...
                    delet_registro = Modalidad.objects.filter(
                        pk__in=id_deletes)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = "Lista de Modalidad Matrícula"
        context['icono_titulo'] = "fas fa-tachometer-alt"
        context['form'] = ModalidadMatriculaForm()
        return context
