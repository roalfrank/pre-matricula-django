...
from .form import EstadoMatriculaForm
from core.preMatricula.mixis import ValidatePermissionRequiredCrudSimpleMixin


class EstadoMatriculaView(LoginRequiredMixin, ValidatePermissionRequiredCrudSimpleMixin, TemplateView):
    template_name = "matricula/matricula-estado/list.html"
    permiso_vista = 'view_estadomatricula'
    permiso_crud = {
        'add': 'add_estadomatricula',
        'change': 'change_estadomatricula',
        'delete': 'delete_estadomatricula',
    }
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'add':
                form = EstadoMatriculaForm(request.POST)
                if form.is_valid():
                    form.save()
                    data['enviado'] = True
            ...
            # Action  cargarDatos - para cargar la datatable de mi vista.
                ...
                data = [i.toJson() for i in EstadoMatricula.objects.all()]
                respuesta = JsonResponse(data, safe=False)
                return respuesta
            # action edit - editando una entidad.
            elif action == 'edit':
                registro = EstadoMatricula.objects.get(
                    pk=int(request.POST['id_edit']))
                form = EstadoMatriculaForm(
            ...
            # action delete, eliminar una registro
                ...
                try:
                    ...
                    delet_registro = EstadoMatricula.objects.filter(
                        pk__in=id_deletes)
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = "Lista de Estado Matrícula"
        context['icono_titulo'] = "fas fa-tachometer-alt"
        context['form'] = EstadoMatriculaForm()
        return context
